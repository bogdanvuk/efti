...
void set_counts(POINT** cur_points,int cur_no_of_points,int flag)
{
  ...
  /* printf("Coef0: %f, Coef1: %f, Coef2: %f\n", coeff_array[1], coeff_array[2], coeff_array[3]); */
  /* printf("Attr0: %f, Attr1: %f, Attr2: %f\n", cur_points[1]->dimension[13], */
  /* 		 cur_points[1]->dimension[31], cur_points[1]->dimension[3]); */
  /* printf("Inst0: %f, Inst1: %f, Inst2: %f\n", cur_points[1]->val, cur_points[2]->val, cur_points[3]->val); */
  if (veryverbose)
	  print_distribution_matrix();
}
...
int stop_splitting()
{ 
  int i,lpt=0,rpt=0,left_cat,right_cat;
